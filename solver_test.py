...
from baselines.defense.DUP_Net.pu_net import PUNet
from model.networks import AutoEncoder  # , Generator, Discriminator
...
class PointTester(object):
    ...
    def set_pre_head(self, mode):
        ...
        - denoising: reserved, but PCNet is not included in this repo.
        """
        if mode == 'sor':
            ...
            raise NotImplementedError('PCNet is not included in this repository.')
            # 后续实现了 PCNet，再改成：
            # pre_head = PCNet()
        else:
            raise NotImplementedError
    ...
    def run_test(self):
        ...
        # I-FGM
        ifgm_attack = IFGM(
            self.source_classifier, self.classifier,
            adv_func=adv_func, clip_func=clip_func,
            budget=budget, step_size=step_size, num_iter=num_iter, dist_metric='l2'
        )
        ...
        mifgm_attack = MIFGM(
            self.source_classifier, self.classifier,
            adv_func=adv_func, clip_func=clip_func,
            budget=budget, step_size=step_size, num_iter=num_iter,
            mu=self.args.mu, dist_metric='l2'
        )
        ...
        pgd_attack = PGD(
            self.source_classifier, self.classifier,
            adv_func=adv_func, clip_func=clip_func, budget=budget,
        ...
        adv_target = torch.Tensor(adv_target).long().to(ori_target.device)  # [B]
