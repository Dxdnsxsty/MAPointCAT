...
class NTXentLoss(torch.nn.Module):
    ...
    def forward(self, zis, zjs):
        ...
        l_pos = torch.diag(similarity_matrix, self.batch_size) # [N]
        r_pos = torch.diag(similarity_matrix, -self.batch_size) # [N]
        positives = torch.cat([l_pos, r_pos]).view(2 * self.batch_size, 1) # [2N, 1]

        negatives = similarity_matrix[self.mask_samples_from_same_repr].view(2 * self.batch_size, -1) # [2N, 2N-2]
